...
class LogsDataProcessor:
    def __init__(
        ...
        pool=1,
        context_token_with_history=True,
    ):
        ...
        self._pool = pool
        self.context_token_with_history = context_token_with_history
    ...
    def _load_df(self, sort_temporally=False):
        ...
        df[df.columns[3:]] = (
            df[df.columns[3:]].astype(str).apply(lambda col: col.str.lower())
        )
        df[df.columns[3:]] = (
            df[df.columns[3:]].astype(str).apply(lambda col: col.str.replace(" ", "-"))
        )
        ...
        df["time:timestamp"] = pd.to_datetime(
            ...
            errors="coerce",  # Set errors='coerce' to handle parsing errors by setting NaT for invalid values
            dayfirst=True,  # You might want to use `dayfirst` based on your date format
        )
        if sort_temporally:
    ...
    def _extract_logs_metadata(self, df):
        ...
        for column in df[df.columns[4:]]:
            context_keys = ["[PAD]", "[UNK]"] + list(df[column].unique())
            res_val = range(len(context_keys))
            coded_resource = dict(
                {f"{column}_word_dict": dict(zip(context_keys, res_val))}
            )
    ...
    def _next_activity_helper_func(self, df):
        ...
        processed_df = pd.DataFrame(
            columns=["case_id", "prefix", "k", "suffix"].extend(self._org_columns[3:])
        )
        idx = 0
        unique_cases = df[case_id].unique()
        for _, case in enumerate(unique_cases):
            act = df[df[case_id] == case][case_name].to_list()
            context_df = df[df[case_id] == case][df.columns[3:]]
            for i in range(len(act) - 1):
                prefix = np.where(i == 0, act[i], " ".join(act[: i + 1]))
                suffix = np.where(i == len(act) - 1, act[-1], " ".join(act[i + 1 :]))
                processed_df.at[idx, "case_id"] = case
                processed_df.at[idx, "prefix"] = prefix
                processed_df.at[idx, "k"] = i
                processed_df.at[idx, "suffix"] = suffix
                for column in context_df:
                    context = context_df[column].to_list()
                    if column == context_df.columns[0]:
                        context_data = np.where(
                            i == len(context) - 1,
                            context[-1],
                            " ".join(context[i + 1 :]),
                        )
                        ...
                        context_data = np.where(
                            i == 0, context[0], " ".join(context[: i + 1])
                        )
                    processed_df.at[idx, column] = context_data
                idx = idx + 1
        return processed_df
    ...
    def _process_next_activity(self, df, train_list, test_list):
        ...
        with Pool(processes=self._pool) as pool:
            processed_df = pd.concat(
                pool.imap(self._next_activity_helper_func, df_split)
            )
        train_df = processed_df[processed_df["case_id"].isin(train_list)]
        test_df = processed_df[processed_df["case_id"].isin(test_list)]
        train_df.to_csv(
            f"{self._dir_path}/{Task.NEXT_ACTIVITY.value}_train.csv", index=False
        )
        test_df.to_csv(
            f"{self._dir_path}/{Task.NEXT_ACTIVITY.value}_test.csv", index=False
        )
    ...
    def _next_time_helper_func(self, df):
        ...
        for _, case in enumerate(unique_cases):
            ...
            for i in range(0, len(act)):
                ...
                if i > 0:
                    latest_diff = datetime.datetime.strptime(
                        time[i], "%Y-%m-%d %H:%M:%S"
                    ) - datetime.datetime.strptime(time[i - 1], "%Y-%m-%d %H:%M:%S")
                if i > 1:
                    recent_diff = datetime.datetime.strptime(
                        time[i], "%Y-%m-%d %H:%M:%S"
    ...
    def _process_next_time(self, df, train_list, test_list):
        ...
        with Pool(processes=self._pool) as pool:
            processed_df = pd.concat(
                pool.imap_unordered(self._next_time_helper_func, df_split)
            )
        train_df = processed_df[processed_df["case_id"].isin(train_list)]
        test_df = processed_df[processed_df["case_id"].isin(test_list)]
        train_df.to_csv(
            f"{self._dir_path}/{Task.NEXT_TIME.value}_train.csv", index=False
        )
    ...
    def _remaining_time_helper_func(self, df):
        ...
        for _, case in enumerate(unique_cases):
            ...
            for i in range(0, len(act)):
                ...
                idx = idx + 1
        processed_df_remaining_time = processed_df[
            [
                "case_id",
                "prefix",
                "k",
    ...
    def _process_remaining_time(self, df, train_list, test_list):
        ...
        with Pool(processes=self._pool) as pool:
            processed_df = pd.concat(
                pool.imap_unordered(self._remaining_time_helper_func, df_split)
            )
        ...
        train_remaining_time.to_csv(
            f"{self._dir_path}/{Task.REMAINING_TIME.value}_train.csv", index=False
        )
        test_remaining_time.to_csv(
            f"{self._dir_path}/{Task.REMAINING_TIME.value}_test.csv", index=False
        )
    ...
    def process_logs(self, task, sort_temporally=False, train_test_ratio=0.80):
        ...
        train_test_ratio = int(
            abs(df["case:concept:name"].nunique() * train_test_ratio)
        )
